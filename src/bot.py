...
from run import Pindle, ShenkEld, Trav, Nihlatak, Arcane, Mephisto
...
class Bot:
    def __init__(self, screen: Screen, game_stats: GameStats, pick_corpse: bool = False):
        ...
        self._mephisto = Mephisto(self._screen, self._template_finder, self._pather, self._town_manager, self._ui_manager, self._char, self._pickit)
        ...
        self._states=['hero_selection', 'town', 'pindle', 'shenk', 'trav', 'nihlatak', 'arcane', 'mephisto']
        self._transitions = [
            ...
            # End run / game
            { 'trigger': 'end_run', 'source': ['shenk', 'pindle', 'nihlatak', 'trav', 'arcane', 'mephisto'], 'dest': 'town', 'before': "on_end_run"},
            { 'trigger': 'end_game', 'source': ['town', 'shenk', 'pindle', 'nihlatak', 'trav', 'arcane', 'mephisto', 'end_run'], 'dest': 'hero_selection', 'before': "on_end_game"},
        ]
    ...
    def on_run_mephisto(self):
        ...
        self._game_stats.update_location("Meph" if self._config.general['discord_status_condensed'] else "Mephisto")
        self._curr_loc = self._mephisto.approach(self._curr_loc)
        if self._curr_loc:
            res = self._mephisto.battle(not self._pre_buffed)
        self._tps_left -= self._mephisto.used_tps
        self._ending_run_helper(res)
